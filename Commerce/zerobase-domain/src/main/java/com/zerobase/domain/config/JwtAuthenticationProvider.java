...
public class JwtAuthenticationProvider {
    ...
    public boolean validateToken(String jwtToken) {
        try {
            Jws<Claims> claimsJws = Jwts.parser().setSigningKey(secretKey).build().parseClaimsJws(jwtToken);
    ...
    public UserVo getUserVo(String token) {
        Claims c = Jwts.parser().setSigningKey(secretKey).build().parseClaimsJws(token).getBody();
        return new UserVo(
                Long.valueOf(Objects.requireNonNull(
                Aes256Util.decrypt(c.getId()))),
